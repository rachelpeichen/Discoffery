...
class ReviewManager {
  ...
  // MARK: - Functions
  func publishReview(shop: inout CoffeeShop, completion: @escaping (Result<String, Error>) -> Void) {

    // MOCK DATA
    let mockUserName = ["Fuku","Rilakkuma","Piske", "Usagi", "Gaspard", "Hamlet"]
    let mockRating = [4.6, 4.4, 3.6, 4.5, 3.3, 3.7, 2.3, 2.7, 4.0, 4.8]
    let mockComment = ["是會一直想光顧的咖啡店。", "很隱密的一家日式咖啡廳，環境很放鬆、很安靜，手沖咖啡很好喝，但品項很少，也沒有特色甜點，只適合辦公或K書。","整體氣氛還不錯，客人有些在辦公有些在喝咖啡聊天，不是非常安靜但也不致於被打擾，個人喜歡這樣的環境，讀書或滑個手機，一個人享受午後時光。", "值得一訪再訪的店，收藏！", "咖啡好喝！價格很平實，但是食物的品質很高！","衣索比亞西達摩單一品種 $230，酸度適中，尾韻烘焙味較明顯。手沖咖啡選擇多，用餐環境舒適，適合在這讀書或放鬆。"]
    ...
    //  Add a sub-collection named "reviews" to every documents in "shopsTaipei"
